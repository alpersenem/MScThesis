...
Accuracy, one of the most common model performance metrics, is an indicator of how well it can distinguish between healthy and faulty motors in the entire data set \cite{grandini2020metrics}. Generally, the number of healthy state data is naturally higher in diagnostic applications that result unbalanced data set. In such a case, evaluating only with the accuracy metric may lead to catastrophic situations. 
\begin{equation}
\text{Accuracy} = \displaystyle\frac{TP + TN}{TP + FP + FN + TN} 
\label{accuracy}	
\end{equation}
\subsection{F-measure}
As a metric based on Recall and Precision, F-Measure allows better inference than accuracy on classification performance, especially on unbalanced datasets \cite{he2009learning}.
\begin{equation}
...
\begin{equation}
\text{Cohen's Kappa} = \displaystyle\frac{c \cdot s-\displaystyle\sum_{k}^{K} p_{k} \cdot t_{k}}{s^{2}-\displaystyle\sum_{k}^{K} p_{k} \cdot t_{k}}
\label{kappa}	
\end{equation}
where:\\
$K\quad$ is the total number classes\\
